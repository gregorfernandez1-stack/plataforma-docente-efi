...
export default function AdminDocumentos() {
  ...
  const subirDocumento = async (e: React.FormEvent) => {
    ...
    if (insertError) {
      setMensaje("El archivo subió, pero no se guardó en la tabla.");
      setSubiendo(false);
      return;
    }
    ...
    <main className="min-h-screen bg-[#F5F7FA] px-6 py-10">
      <section className="max-w-3xl mx-auto bg-white p-8 rounded-2xl shadow">
        ...
        <p className="text-gray-600 mb-8">
          Sube documentos para que los docentes puedan consultarlos desde la biblioteca curricular.
        </p>

        <form onSubmit={subirDocumento} className="space-y-5">
          <div>
            ...
            <input
              ...
              placeholder="Ej: Diseño Curricular Nivel Primario"
            />
          </div>
          ...
          {mensaje && (
            <p className="font-semibold text-[#003B7A]">{mensaje}</p>
          )}
